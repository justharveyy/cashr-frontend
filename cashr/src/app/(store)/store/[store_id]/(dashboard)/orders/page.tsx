      ...
      <section className="mb-8">
        <div className="flex justify-between items-end mb-6">
          <div>
            ...
            <h2 className="text-2xl font-bold text-on-surface">Orders</h2>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="p-5 rounded-xl bg-white border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[10px] px-2 py-1 rounded bg-emerald-50 text-emerald-700 font-semibold">Paid</span>
            </div>
            <h3 className="text-3xl font-bold text-on-surface leading-tight">
              {loading ? <span className="inline-block h-8 w-40 bg-slate-100 rounded animate-pulse" /> : `₫${netRevenue.toLocaleString("vi-VN")}`}
            </h3>
            <p className="text-xs text-slate-400 mt-2">{paid.length} completed orders</p>
          </div>
          ...
          <div className="p-5 rounded-xl bg-white border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Awaiting Payment</p>
              </div>
              <span className="text-[10px] px-2 py-1 rounded bg-amber-50 text-amber-700 font-semibold">Pending</span>
            </div>
            <h3 className="text-3xl font-bold text-on-surface leading-tight">
              {loading ? <span className="inline-block h-8 w-20 bg-slate-100 rounded animate-pulse" /> : pending.length}
            </h3>
            <p className="text-xs text-slate-400 mt-2">
              ₫{loading ? "—" : pending.reduce((s, o) => s + o.total_amount, 0).toLocaleString("vi-VN")} outstanding value
            </p>
          </div>
          ...
          <div className="p-5 rounded-xl bg-white border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[10px] px-2 py-1 rounded bg-slate-100 text-slate-600 font-semibold">All Time</span>
            </div>
            <h3 className="text-3xl font-bold text-on-surface leading-tight">
              {loading ? <span className="inline-block h-8 w-16 bg-slate-100 rounded animate-pulse" /> : total}
            </h3>
            <p className="text-xs text-slate-400 mt-2">Includes paid, pending and cancelled</p>
          </div>
        </div>
      </section>
